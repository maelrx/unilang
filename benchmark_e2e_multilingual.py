...
def run_benchmark(lang_code: str, lang_name: str, text: str, runtime, enabled: bool, make_ctx) -> BenchmarkResult:
    """Run full benchmark for one language configuration."""
...
def run_session_memory_benchmark(lang_code: str, lang_name: str, text: str, runtime, enabled: bool, turns: int = 5, make_ctx=None) -> float:
    """Simulate a multi-turn conversation and measure memory growth."""
    gc.collect()
    tracemalloc.start()
...
def main():
    print("=" * 80)
    print("  UNILANG E2E MULTILINGUAL BENCHMARK")
    ...
    def make_session_ctx(lang_code: str) -> SessionContext:
        config = LanguageMediationConfig(
            ...
            translator=TranslatorConfig(provider="mock"),
    ...
    for lang_code, lang_name, text in LANGUAGES:
        ...
        cache_in = "✓" if r.cache_hit_input else "✗"
        cache_out = "✓" if r.cache_hit_output else "✗"
        det_ok = "✓" if r.detection_correct else "✗"
        norm = "✓" if r.input_normalized else "="
        out_ok = "✓" if r.output_localized else "="
        tool_ok = "✓" if r.tool_mediated else "="
    ...
    # ── Memory: Multi-turn session simulation ───────────────────────────────
...
""")
    ...
    for r in results_enabled:
        norm_flag = "✓" if r.input_normalized else "="
        print(f"{r.language:<8} {norm_flag:>8} {r.input_latency_ms:>8.2f} {r.output_latency_ms:>8.2f} {r.tool_latency_ms:>9.2f} {norm_flag:>6} {'✓' if r.cache_hit_input else '✗':>7} {'✓' if r.cache_hit_output else '✗':>8}")
    print("-" * 80)
    ...
    output_file = f"/tmp/unilang_benchmark_{timestamp}.json"
